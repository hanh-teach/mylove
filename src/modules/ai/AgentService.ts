...
class AgentService {
  ...
  updateChangeStatus(planId: string, changeId: string, status: 'approved' | 'rejected'): void {
    const plans = this.getPlans();
    const planIndex = plans.findIndex(p => p.id === planId);
    if (planIndex === -1) return;

    let found = false;
    for (let t = 0; t < plans[planIndex].tasks.length; t++) {
      const task = plans[planIndex].tasks[t];
      if (task.proposedChanges) {
        const cIdx = task.proposedChanges.findIndex(c => c.id === changeId);
        if (cIdx !== -1) {
          task.proposedChanges[cIdx].status = status;
          found = true;
          ...
          if (status === 'approved') {
            this.addActivity({
              ...
              details: `Đã duyệt đề xuất: ${task.proposedChanges[cIdx].description}`,
              planId,
              taskId: task.id,
              level: 'success'
            });
            ...
            this.addActivity({
              ...
              details: `Đã từ chối đề xuất: ${task.proposedChanges[cIdx].description}`,
              planId,
              taskId: task.id,
              level: 'info'
            });
          ...
          const remainingPending = task.proposedChanges.filter(c => c.status === 'pending').length;
  ...
  approveAllChanges(planId: string): void {
    const plans = this.getPlans();
    const planIndex = plans.findIndex(p => p.id === planId);
    if (planIndex === -1) return;

    plans[planIndex].tasks.forEach(task => {
      if (task.proposedChanges) {
        task.proposedChanges.forEach(change => {
          if (change.status === 'pending') {
            ...
            this.addActivity({
              ...
              timestamp: new Date().toISOString(),
              action: 'Change Approved',
              details: `Đã duyệt đề xuất: ${change.description}`,
              planId,
              taskId: task.id,
              level: 'success'
            });

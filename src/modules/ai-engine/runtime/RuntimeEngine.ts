...
const workflowPlanner = {
  plan: async (enrichedGoal: string, options: any): Promise<PlanningResult> => {
    return {
      isValidDAG: true,
      workflow: {
        id: 'wf-mock-123',
  ...
  public async runGoal(goal: string, userId?: string): Promise<WorkflowResult> {
    ...
    try {
      ...
      const decisionResult = {
        trace: {
          log: () => console.log('[DecisionEngine Mock] Decided best option.'),
          getTrace: () => 'Decided best option based on Balanced presets'
        }
      };

...
export const VersionHistoryPanel: React.FC<VersionHistoryPanelProps> = ({
  ...
  return (
    <div className="w-full sm:w-[420px] h-full bg-white/98 backdrop-blur-md shadow-2xl border-l border-rose-100 flex flex-col z-50 animate-slideLeft">
      {/* Header */}
        ...
        <div className="w-full lg:w-1/2 border-r border-rose-100 overflow-y-auto p-4 space-y-4">
          {versions.length === 0 ? (
            <div className="text-center py-12 text-slate-400 text-xs">
              Chưa có lịch sử phiên bản nào. Hãy tiếp tục soạn thảo, hệ thống sẽ tự động lưu!
            </div>
          ) : (
            <>
              {todayList.length > 0 && (
                <div>
                  <div className="text-[11px] font-bold text-rose-600 uppercase tracking-wider mb-2">Hôm nay</div>
              ...
              {yesterdayList.length > 0 && (
                <div>
                  <div className="text-[11px] font-bold text-rose-600 uppercase tracking-wider mb-2 mt-4">Hôm qua</div>
                  <div className="space-y-2">
                    {yesterdayList.map(v => (
                      <VersionItem 
                        key={v.id} 
              ...
              {earlierList.length > 0 && (
                <div>
                  <div className="text-[11px] font-bold text-rose-600 uppercase tracking-wider mb-2 mt-4">Cũ hơn</div>
                  <div className="space-y-2">
                    {earlierList.map(v => (
                      <VersionItem 
                        key={v.id} 
            ...
            <div className="flex flex-col h-full justify-between gap-4">
              <div>
                <div className="flex items-center justify-between mb-3 pb-2 border-b border-rose-200">
                  <span className="text-xs font-bold text-rose-900">Xem trước v{previewVersion.versionNumber}</span>
                  <span className="text-[10px] bg-rose-100 text-rose-700 px-2 py-0.5 rounded-full font-medium">
                    {getTypeText(previewVersion.type)}
                  </span>
                </div>
                <div className="bg-white rounded-xl p-4 border border-rose-100 shadow-xs space-y-3 font-serif">
                  <h3 className="text-base font-bold text-rose-950 border-b border-rose-100 pb-1">
                    {previewVersion.document.title || 'Không có tiêu đề'}
                  </h3>
                  <div className="space-y-2 text-xs text-slate-700 max-h-[220px] overflow-y-auto pr-1">
                    {previewVersion.document.blocks.map(b => (
                      <p key={b.id} className="leading-relaxed">
                        {b.type === 'heading' ? <strong dangerouslySetInnerHTML={{ __html: sanitizeHtml(b.content) }} /> : <span dangerouslySetInnerHTML={{ __html: sanitizeHtml(b.content) }} />}
                      </p>
                    ))}
                  </div>
                </div>
                <div className="mt-3 text-[11px] text-slate-500 italic">
                  Chi tiết: {previewVersion.summary}
                </div>
              </div>

...
export const WorkflowSidebar: React.FC<WorkflowSidebarProps> = ({ onClose, isMobileDrawer = false }) => {
  ...
  // Subscribe to WorkflowController changes for real-time reactivity
  useEffect(() => {
    return WorkflowController.subscribe(() => {
  ...
  // Reset selected step when active template changes
  useEffect(() => {
    setSelectedStep(null);
  }, [template.id]);
  const progress = WorkflowController.getProgress();
  const isPaused = WorkflowController.getIsPaused();
  const isRunning = WorkflowController.getIsRunning();
  const timelineLogs = WorkflowController.getTimelineLogs();

  return (
    <div className={`bg-white/98 backdrop-blur-md flex flex-col h-full border-l border-rose-100 ${isMobileDrawer ? 'w-full rounded-t-3xl shadow-2xl max-h-[85vh]' : 'w-full lg:w-[360px] shadow-lg'}`} id="smart-workflow-sidebar">
      {/* Header */}
      <div className="bg-rose-50/70 border-b border-rose-100 px-4 py-3.5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2 text-rose-950 font-serif font-bold text-base">
          <Sparkles size={18} className="text-rose-600 animate-pulse" />
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gradient-to-br from-rose-50 to-pink-50/70 border border-rose-200/80 rounded-2xl p-4 space-y-3 shadow-2xs">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[10px] font-bold text-rose-600 uppercase tracking-wider">Quy trình hoạt động</span>
              ...
              <div className="flex items-center gap-2 pt-1">
                {!isRunning ? (
                  <button
                    onClick={() => WorkflowController.restart()}
                    className="flex-1 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-xs transition-all"
                  >
                    <RefreshCw size={13} className="animate-spin" /> Chạy lại quy trình
                  </button>
                ) : (
                  <>
                    {isPaused ? (
            ...
            <div className="space-y-2">
              <h5 className="font-bold text-xs text-rose-900 uppercase tracking-wider">Các bước trong quy trình</h5>
              <div className="space-y-2">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                        ...
                        step.status === 'running' ? 'bg-rose-600 text-white animate-pulse' :
                        step.status === 'failed' ? 'bg-rose-600 text-white' : 'bg-slate-200 text-slate-600'
                      }`}>
                        {idx + 1}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {step.status === 'running' && <span className="text-[10px] bg-rose-200 text-rose-800 px-2 py-0.5 rounded font-bold">Đang chạy</span>}
                      {step.status === 'failed' && (
                        <button
            ...
            {selectedStep && (
              <div className="bg-rose-50/90 border border-rose-200 rounded-2xl p-4 space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setSelectedStep(null)} className="text-rose-500 hover:text-rose-800 text-xs font-bold">Đóng</button>
                </div>
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <div className="bg-white p-2 rounded-xl border border-rose-100">
                    <span className="text-[10px] text-slate-500 block">Công nghệ / Mô hình</span>
                    <span className="font-bold text-slate-800">{selectedStep.provider}</span>
                  </div>
                  <div className="bg-white p-2 rounded-xl border border-rose-100">
                    <span className="text-[10px] text-slate-500 block">Thời gian ước tính</span>
                  ...
                  <div className="bg-white p-2 rounded-xl border border-rose-100">
                    ...
                    <span className="font-bold text-rose-700">
                      {selectedStep.status === 'completed' ? 'Hoàn thành' :
                       selectedStep.status === 'running' ? 'Đang chạy' :
                       selectedStep.status === 'failed' ? 'Thất bại' : 'Đang chờ'}
                    </span>
                  </div>
                </div>
                {selectedStep.outputSummary && (
                  <p className="text-[11px] text-emerald-800 font-medium bg-emerald-50 p-2 rounded-lg border border-emerald-200">
                    ✓ Kết quả: {selectedStep.outputSummary}
                  </p>
                )}
              </div>
            )}
            ...
            <div className="space-y-2 pt-2 border-t border-rose-100">
              <h5 className="font-bold text-xs text-rose-900 uppercase tracking-wider">Nhật ký quy trình</h5>
        ...
        {activeTab === 'templates' && (
          <div className="space-y-3">
            <h5 className="font-bold text-xs text-rose-900 uppercase tracking-wider">Kho mẫu quy trình sáng tạo</h5>
        ...
        {activeTab === 'history' && (
          <div className="space-y-3">
            <h5 className="font-bold text-xs text-rose-900 uppercase tracking-wider">Lịch sử chạy quy trình</h5>
            <div className="space-y-2.5">
              <div 
                onClick={() => {
                  WorkflowController.setActiveTemplate('creative_story');
                  setActiveTab('progress');
                }}
                className={`p-3.5 rounded-2xl border transition-all cursor-pointer hover:shadow-md hover:border-rose-300 ${
                  template.id === 'creative_story' ? 'bg-rose-50/70 border-rose-200' : 'bg-white border-slate-100'
                } space-y-1`}
              >
                <div className="flex items-center justify-between text-xs font-bold text-slate-800">
                  <span className="font-serif text-rose-950 font-bold">Story & Media Keepsake v1</span>
                  <span className="text-emerald-600 font-bold">Hoàn thành</span>
                </div>
                <p className="text-[10px] text-slate-500">Hôm qua • Hoàn thành xuất sắc 6 bước</p>
                <div className="text-[10px] text-rose-600/80 font-semibold pt-1 flex items-center gap-1">
                  <span>Nhấp để tải lại quy trình này</span>
                  <ChevronRight size={10} />
                </div>
              </div>
              <div 
                onClick={() => {
                  WorkflowController.setActiveTemplate('birthday_card');
                  setActiveTab('progress');
                }}
                className={`p-3.5 rounded-2xl border transition-all cursor-pointer hover:shadow-md hover:border-rose-300 ${
                  template.id === 'birthday_card' ? 'bg-rose-50/70 border-rose-200' : 'bg-white border-slate-100'
                } space-y-1`}
              >
                <div className="flex items-center justify-between text-xs font-bold text-slate-800">
                  <span className="font-serif text-rose-950 font-bold">Birthday Surprise</span>
                  <span className="text-amber-600 font-bold">Đang tạm dừng</span>
                </div>
                <p className="text-[10px] text-slate-500">Hôm nay • Đã hoàn tất 2 trên 4 bước</p>
                <div className="text-[10px] text-rose-600/80 font-semibold pt-1 flex items-center gap-1">
                  <span>Nhấp để tải lại quy trình này</span>
                  <ChevronRight size={10} />
                </div>

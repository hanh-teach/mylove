  ...
  constructor(
    ...
    if (layer) {
      if (this.propertyName.startsWith('metadata.')) {
        ...
        const validKeys: (keyof ILayer)[] = [
          'id', 'type', 'name', 'visible', 'locked', 'opacity',
          'rotation', 'x', 'y', 'width', 'height', 'zIndex',
          'parentId', 'children', 'metadata'
        ];
        if (validKeys.includes(this.propertyName as keyof ILayer)) {
          const key = this.propertyName as keyof ILayer;
          this.prevValue = layer[key];
        } else {
          console.warn(`[HistoryManager] Invalid propertyName: ${this.propertyName}`);
  ...
  constructor(
    ...
    if (layer) {
      Object.keys(updates).forEach(key => {
        ...
        if (validKeys.includes(key as keyof ILayer)) {
          this.prevValues[key] = layer[key as keyof ILayer];
        } else {
          console.warn(`[HistoryManager] Invalid property: ${key} in batch updates`);
          this.prevValues[key] = undefined;
        }
      });
    }
  }

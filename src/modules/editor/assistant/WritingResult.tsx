...
import { Sparkles, Check, ArrowDown, RefreshCw, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="mt-4 p-4 rounded-xl bg-rose-50/50 border border-rose-200 flex flex-col gap-3 animate-fadeIn">
      ...
      {!isGenerating && resultText && (
        <>
          {isSimulated && (
            <div className="bg-amber-100 border border-amber-300 rounded-lg p-2 flex items-start gap-2 text-amber-800 text-xs mb-1">
              <AlertTriangle size={14} className="mt-0.5 shrink-0 text-amber-600" />
              <div>
                <span className="font-bold block">Kết quả giả lập (Simulated)</span>
                <span className="opacity-90 leading-tight block mt-0.5">Hệ thống AI hiện không khả dụng. Đây là kết quả tạo tự động dự phòng.</span>
              </div>
            </div>
          )}

    ...
    this.projects = this.projects.map((p) => {
      if (p.id === this.activeProjectId) {
        ...
        if (updates.checklist || updates.progress !== undefined || updates.content) {
          ...
          if (!newProject.content.title) priorityTasks.push({ id: `${newProject.id}-pr1`, label: 'Đặt tên cho dự án', priority: 5, impact: 'Định hình chủ đề', completed: false });
          if ((newProject.content.wordCount || 0) < 100) priorityTasks.push({ id: `${newProject.id}-pr2`, label: 'Hoàn thiện nội dung chính', priority: 4, impact: 'Tăng 30% tiến độ', completed: false });
          if (!newProject.memoriesCount) priorityTasks.push({ id: `${newProject.id}-pr3`, label: 'Thu thập ảnh kỷ niệm', priority: 3, impact: 'Làm sinh động dự án', completed: false });
          
          const combinedTasks = [...priorityTasks, ...(newProject.priorityTasks || []).filter(t => !t.id.includes('-pr'))];
          const uniqueTasks = Array.from(new Map(combinedTasks.map(t => [t.id, t])).values());
          newProject.priorityTasks = uniqueTasks.slice(0, 4);

...
function normalizeProjectStrings(p: Project): Project {
  if (!p) return p;
  return {
    ...p,
    ...
    content: p.content ? {
      ...p.content,
      title: normalizeString(p.content.title),
      message: normalizeString(p.content.message),
      placedItems: p.content.placedItems || [],
      scene: p.content.scene || 'rose',
      bgStyle: p.content.bgStyle || 'solid',
      fontStyle: p.content.fontStyle || 'playfair',
      textColor: p.content.textColor || 'default',
    } : { title: '', message: '', placedItems: [], scene: 'rose', bgStyle: 'solid', fontStyle: 'playfair', textColor: 'default', wordCount: 0 },
    tags: p.tags || [],
    healthStatus: p.healthStatus || { content: 100, media: 100, timeline: 100, overall: 'excellent' },
    insight: p.insight || { totalMemories: 0, totalAssets: 0, totalEvents: 0, aiDrafts: 0, manualDrafts: 0, completionPrediction: '---' },
    checklist: p.checklist || [],
    recentActivity: p.recentActivity || [],
    coachSuggestions: p.coachSuggestions || [],
    priorityTasks: p.priorityTasks || [],
    dailyFocus: p.dailyFocus || [],
    workspaceInsights: p.workspaceInsights || [],
    productivity: p.productivity || { weeklyProjects: 0, weeklyHours: 0, newMemories: 0, aiAssists: 0, completedProjects: 0 },
    intelligenceSettings: p.intelligenceSettings || { showInsights: true, showProductivity: true, showSmartReminders: true, activeCoach: true },
  };
}

export class ProjectRepository {
  public static getAll(): Project[] {
    if (typeof window === 'undefined') return INITIAL_SAMPLE_PROJECTS.map(normalizeProjectStrings);
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
          return parsed.map(normalizeProjectStrings);
        }
      }
    } catch (e) {
    ...
    const normalized = INITIAL_SAMPLE_PROJECTS.map(normalizeProjectStrings);
  ...
  public static saveAll(projects: Project[]): void {
    ...
    try {
      const normalized = projects.map(normalizeProjectStrings);

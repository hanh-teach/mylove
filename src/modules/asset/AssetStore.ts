...
const ASSETS_SEEDED_KEY = 'love_note_project_assets_seeded_v59';
...
export class AssetStore {
  ...
  private seededProjects: Set<string> = new Set();
  private listeners: Set<() => void> = new Set();
  private isHydrating: boolean = false;
  private hasWrittenDuringHydration: boolean = false;
  ...
  private loadAssets() {
    try {
      const storedSeeded = localStorage.getItem(ASSETS_SEEDED_KEY);
      if (storedSeeded) {
        try {
          this.seededProjects = new Set(JSON.parse(storedSeeded));
        } catch {
          this.seededProjects = new Set();
        }
      }
  ...
  private async hydrateFromIndexedDb() {
    try {
      const storedSeeded = await idbAdapter.get<string>(ASSETS_SEEDED_KEY);
      if (storedSeeded) {
        try {
          this.seededProjects = new Set(JSON.parse(storedSeeded));
        } catch {
      ...
      const stored = await idbAdapter.get<string>(ASSETS_STORAGE_KEY);
      if (stored) {
        if (this.hasWrittenDuringHydration) {
          console.warn('Skipping IDB hydration because a newer write occurred in memory');
          return;
        }
        this.assets = JSON.parse(stored);
        this.notify();
      ...
      console.error('Failed to hydrate project assets from IDB:', e);
    } finally {
  ...
  private saveAssets() {
    ...
    const serialized = JSON.stringify(this.assets);
    const serializedSeeded = JSON.stringify(Array.from(this.seededProjects));
    try {
      // Prioritize IDB
      idbAdapter.set(ASSETS_STORAGE_KEY, serialized).catch(e => {
        console.error('IDB save failed, falling back to LS', e);
      });
      idbAdapter.set(ASSETS_SEEDED_KEY, serializedSeeded).catch(() => {});
      // Fallback/Backup to LS
      localStorage.setItem(ASSETS_STORAGE_KEY, serialized);
      localStorage.setItem(ASSETS_SEEDED_KEY, serializedSeeded);
    } catch (e) {
  ...
  public getAssets(projectId?: string): ProjectAsset[] {
    ...
    if (projectAssets.length === 0 && !this.seededProjects.has(projectId)) {
  ...
  public addAsset(asset: Omit<ProjectAsset, 'id' | 'createdAt' | 'updatedAt' | 'version' | 'status'> & { id?: string }): ProjectAsset {
    ...
    if (asset.projectId) {
      this.seededProjects.add(asset.projectId);
    }
  ...
  public restoreAllFromTrash(projectId?: string): void {
    this.assets = this.assets.map(a => {
      if (projectId && a.projectId !== projectId) return a;
      if (a.status === 'trash') {
        return { ...a, status: 'active', updatedAt: Date.now() };
      }
      return a;
    });
    this.saveAssets();

...
export class AssetManager {
  ...
  public registerAsset(data: {
    ...
    tags?: string[];
    aiGenerated?: boolean;
    provider?: 'local' | 'system' | 'external' | 'gemini' | 'ai-studio' | 'supabase';
    description?: string;
    mimeType?: string;
  }): ProjectAsset {
   ...
   */
  public queryAssets(options?: Partial<AssetFilterOptions>): ProjectAsset[] {
    const opts = options || {};
    let list = assetStore.getAssets(opts.projectId);

    // Status filter (active vs trash)
    const targetStatus = opts.status || (opts.category === 'trash' ? 'trash' : 'active');
    list = list.filter(a => a.status === targetStatus);

    // Category filter
    if (opts.category && opts.category !== 'all' && opts.category !== 'trash') {
      if (opts.category === 'favorite') {
      ...
      } else if (opts.category === 'ai') {
        list = list.filter(a => a.aiGenerated || a.type === 'ai');
      } else if (opts.category === 'icon') {
        list = list.filter(a => a.type === 'icon' || a.type === 'sticker');
      } else {
        list = list.filter(a => a.type === opts.category);
      }
    }

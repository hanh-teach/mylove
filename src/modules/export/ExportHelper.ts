...
export async function captureElementToCanvas(
  ...
  try {
    ...
    const targetIsFixed = isFixedPositioningInEffect(targetElement);

    const captureX = options.x !== undefined ? options.x : (isStandaloneArtboard ? 0 : (targetIsFixed ? rect.left : rect.left + window.scrollX));
    const captureY = options.y !== undefined ? options.y : (isStandaloneArtboard ? 0 : (targetIsFixed ? rect.top : rect.top + window.scrollY));
  ...
  try {
    ...
    const canvas = await captureElementToCanvas(targetElement, {
      ...
      onClone: (clonedDoc) => {
        ...
        // Show clean export text if inside editor mode
        const exportTexts = clonedDoc.querySelectorAll('[data-export-text-content="true"]');
        exportTexts.forEach((el) => {
          (el as HTMLElement).classList.remove('opacity-0', 'pointer-events-none');

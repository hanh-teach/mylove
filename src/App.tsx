...
import { Heart, Flower, Leaf, Star, Smile, Gift, Sparkles, Cake, Users, Flower2, RotateCcw, Music, Type, Settings, PenTool, Check, Palette, Plus, Minus, VolumeX, Coffee, TreePine, Video, Loader2, Play, Download, AlertCircle, Film, Clock, Wand2, Folder, Share2, Scissors } from 'lucide-react';
...
import { AudioTrimmerModal } from './components/audio/AudioTrimmerModal';
...
import { WorkspaceDashboard, ProjectWorkspaceProvider, useProjectWorkspace, Project } from './modules/workspace';
...
import { ExportStudioModal } from './components/editor/export/ExportStudioModal';
...
const FramedPhotoRender: React.FC<{
  ...
  animate?: any;
  transition?: any;
}> = ({ src, frameShape, animate, transition }) => {
  const [framedSrc, setFramedSrc] = useState<string>(src);

  useEffect(() => {
    let active = true;
    getFramedPhotoUrl(src, frameShape, 400).then((url) => {
  ...
  return (
    ...
    >
      <img
        ...
        className="w-32 h-32 sm:w-44 sm:h-44 object-contain transition-all duration-300 pointer-events-none drop-shadow-md"
        draggable={false}
...
function AppContent() {
  ...
  const downloadAnimatedCardVideo = async () => {
    ...
    try {
      ...
      for (const item of placedItems) {
        const decor = decorRegistry[item.type];
        if (decor && decor.type === 'image' && typeof decor.content === 'string') {
          if (!decorImagesMap.has(item.type)) {
            const img = new Image();
            img.crossOrigin = 'anonymous';
            await new Promise<void>((res) => {
              img.onload = () => res();
      ...
      for (let frame = 0; frame < totalFrames; frame++) {
        ...
        if (cleanTitle) {
          drawShadowText(cleanTitle, 640, cleanMessage ? 280 : 360, `bold 48px ${chosenFont}`);
        }
        if (cleanMessage) {
          drawShadowText(cleanMessage, 640, cleanTitle ? 400 : 360, `28px ${chosenFont}`);
        }
        ctx.restore();
          ...
          >
            {customImg && !decor ? (
              ...
              <motion.div 
                ...
                style={(item as any).frameShape ? frameStyle : {}}
              >
                <img 
            ...
            <div data-html2canvas-ignore="true" className={`absolute -top-12 -right-6 flex flex-col gap-1 transition-opacity bg-white/70 backdrop-blur-sm p-1 rounded-lg z-60 shadow-md ${selectedDecorId === item.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
              <button onClick={() => scaleDecor(item.id, 0.2)} className="text-xs bg-white rounded-full w-5 h-5 flex items-center justify-center hover:bg-rose-100 transition-colors" title="Phóng to">+</button>
              <button onClick={() => scaleDecor(item.id, -0.2)} className="text-xs bg-white rounded-full w-5 h-5 flex items-center justify-center hover:bg-rose-100 transition-colors" title="Thu nhỏ">-</button>
              <button onClick={() => rotateDecor(item.id, 15)} className="text-xs bg-white rounded-full w-5 h-5 flex items-center justify-center hover:bg-rose-100 transition-colors" title="Xoay 15°">↻</button>
              <button 
                onClick={() => cycleFrameShape(item.id)} 
              ...
              >
                ...
                <span className="absolute right-full mr-2 top-1/2 -translate-y-1/2 bg-purple-900 text-white text-[10px] px-2 py-0.5 rounded opacity-0 group-hover/frame:opacity-100 whitespace-nowrap pointer-events-none shadow-md z-70">
                  Khung: {shapeConfig.label}
                </span>
              </button>
              <button onClick={() => cycleAnimation(item.id)} className="text-xs bg-emerald-100 text-emerald-800 rounded-full w-5 h-5 flex items-center justify-center relative group/anim" title="Hiệu ứng động">
                ✨
              ...
              <button onClick={() => removeDecor(item.id)} className="text-xs bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center hover:bg-red-600 transition-colors" title="Xóa">x</button>
            </div>
          </motion.div>
        );
      })}
        ...
        {showSettingsMenu && (
          ...
          >
            ...
            <div className="max-w-7xl mx-auto px-2 sm:px-4 py-2 flex flex-wrap items-center justify-center gap-1.5 sm:gap-2.5 overflow-x-auto no-scrollbar">
              ...
              <button 
                onClick={cycleFont} 
                className="p-2 sm:p-2.5 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-800 transition-all flex items-center justify-center shrink-0 border border-indigo-200 cursor-pointer active:scale-95 shadow-xs"
                title="Đổi phông chữ"
              >
            ...
            <AnimatePresence mode="wait">
              ...
              {showPalette && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, height: 'auto' }}
                  exit={{ opacity: 0, height: 0 }}
                  className="bg-pink-50/90 border-t border-pink-100 px-3 py-2 text-xs"
                >
                  <div className="max-w-7xl mx-auto flex flex-col gap-2">
                    <div className="flex items-center gap-3">
                      <span className="font-semibold text-pink-900 shrink-0">Thêm họa tiết trang trí:</span>
                      <div className="flex items-center gap-1 bg-white px-2 py-0.5 rounded-lg border border-pink-200">
                        <span className="text-[11px] text-slate-600 font-medium">Màu:</span>
                      ...
                      <button onClick={clearAllDecor} className="ml-auto text-xs bg-red-100 hover:bg-red-200 text-red-700 px-2.5 py-1 rounded-lg font-medium transition-colors cursor-pointer">
                        Xóa tất cả họa tiết
                      </button>
                    </div>
                    <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto pt-1">
                      {(Object.keys(decorRegistry) as DecorType[]).map(type => {
                        ...
                        return (
                          <button key={type} onClick={() => addDecor(type)} className="hover:scale-110 transition-transform p-1.5 bg-white rounded-xl hover:bg-pink-100 cursor-pointer border border-pink-200">
                            {decor.type === 'icon' ? (
                              <div style={{ color: decorColor }}><decor.content size={20} /></div>
                            ) : (
                              <img src={decor.content as string} alt={type} className="w-7 h-7 object-contain" />
                            )}
                          </button>
                        );
                      })}

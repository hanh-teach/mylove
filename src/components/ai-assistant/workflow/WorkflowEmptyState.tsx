...
import { Sparkles, Play, Film, Heart, BookOpen, BrainCircuit } from 'lucide-react';
...
export const WorkflowEmptyState: React.FC<Props> = ({ onStartWorkflow }) => {
  ...
  const [aiAssistEnabled, setAiAssistEnabled] = useState(() => {
    return localStorage.getItem('lovenote-ai-assist-enabled') !== 'false';
  });

  useEffect(() => {
    const handleSettingsChange = () => {
      setAiAssistEnabled(localStorage.getItem('lovenote-ai-assist-enabled') !== 'false');
    };
    window.addEventListener('lovenote-ai-settings-changed', handleSettingsChange);
    return () => window.removeEventListener('lovenote-ai-settings-changed', handleSettingsChange);
  }, []);
  ...
  if (!aiAssistEnabled) {
    return (
      <div className="bg-slate-50 rounded-2xl p-10 border border-dashed border-slate-300 text-center space-y-4 max-w-xl mx-auto my-6 animate-in fade-in duration-500">
        <div className="w-16 h-16 bg-slate-100 text-slate-400 rounded-3xl flex items-center justify-center mx-auto shadow-sm">
          <BrainCircuit size={32} />
        </div>
        <div className="space-y-2">
          <h3 className="text-base font-black text-slate-700">Trợ lý AI đang tạm nghỉ</h3>
          <p className="text-xs text-slate-500 leading-relaxed max-w-xs mx-auto">
            Tính năng Workflow tự động yêu cầu Trợ lý AI Chủ động. Bạn có thể kích hoạt lại tính năng này trong phần <b>Cài đặt AI</b> của hệ thống.
          </p>
        </div>
      </div>
    );

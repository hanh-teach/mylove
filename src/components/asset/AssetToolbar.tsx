...
import { Upload, ArrowUpDown, Trash2, RotateCcw } from 'lucide-react';
...
export const AssetToolbar: React.FC<AssetToolbarProps> = ({
  ...
  return (
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-4 bg-white border-b border-slate-200">
      ...
      {/* Right: Actions (Sort, Upload / Restore All / Clear Trash) */}
      <div className="flex items-center gap-2 shrink-0 justify-end flex-wrap">
        ...
        {/* Trash Actions or Upload Button */}
        {isTrashView ? (
          <div className="flex items-center gap-2">
            {onRestoreAll && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 text-white font-bold text-xs transition-colors shadow-xs active:scale-95"
                title="Khôi phục tất cả tài nguyên trong thùng rác"
              >
                <RotateCcw size={14} />
            ...
            {onClearTrash && (
              <button
                ...
                disabled={totalAssetsCount === 0}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 disabled:opacity-40 text-white font-bold text-xs transition-colors shadow-xs active:scale-95"
                title="Xóa vĩnh viễn tất cả tệp trong thùng rác"
              >
                <Trash2 size={14} />

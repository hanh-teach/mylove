...
export const AssetCard: React.FC<AssetCardProps> = ({
  ...
  return (
    <div
      ...
      className={`group relative bg-white rounded-2xl border transition-all duration-200 cursor-pointer flex flex-col hover:shadow-lg overflow-visible ${
        isSelected
    ...
    >
      ...
      <div className="relative aspect-4/3 w-full bg-slate-100 overflow-hidden flex items-center justify-center">
        ...
        {/* Trash Badge Overlay */}
        {isTrash && (
          <span className="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-slate-900/85 backdrop-blur-md text-white text-[10px] font-bold flex items-center gap-1 shadow-xs border border-white/20">
      ...
      <div className="p-3 flex-1 flex flex-col justify-between">
          ...
          <div className="flex items-start justify-between gap-1.5">
            ...
            <div onClick={(e) => e.stopPropagation()}>
              <AssetMenu
                ...
                onRestore={() => onRestore ? onRestore(asset) : assetManager.restoreAsset(asset.id)}
                onDeletePermanentlyRequest={() => onDeletePermanentlyRequest ? onDeletePermanentlyRequest(asset) : (confirm(`Bạn có chắc muốn xóa vĩnh viễn "${asset.title}"?`) && assetManager.deletePermanently(asset.id))}
              />
            </div>
          </div>
        ...
        {/* Quick Trash Action Buttons or Tags */}
        {isTrash ? (
          <div className="flex items-center gap-1.5 mt-2.5 pt-2 border-t border-slate-100">
            <button
              ...
              className="flex-1 py-1.5 px-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold text-[11px] flex items-center justify-center gap-1 border border-emerald-200 transition-colors shadow-2xs active:scale-95"
              title="Khôi phục tài nguyên này"
            >
              <RotateCcw size={12} />
              <span>Khôi phục</span>
            </button>
            <button
              ...
              onClick={(e) => {
                ...
                if (onDeletePermanentlyRequest) {
                  onDeletePermanentlyRequest(asset);
                } else {
              ...
              className="p-1.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 font-bold text-[11px] flex items-center justify-center border border-rose-200 transition-colors shadow-2xs active:scale-95 shrink-0"
              title="Xóa vĩnh viễn"
            >
              <Trash2 size={13} />
            </button>

...
interface AssetMenuProps {
  ...
  onRename: () => void;
  onRestore?: () => void;
  onDeletePermanentlyRequest?: () => void;
}
...
export const AssetMenu: React.FC<AssetMenuProps> = ({
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-1 w-48 rounded-2xl bg-white border border-slate-200 shadow-2xl text-xs py-1 z-50 divide-y divide-slate-100">
          {!isTrash ? (
            <>
              <div className="py-1">
                <button
                ...
                <button
                  onClick={(e) => {
                    ...
                    if (onDeletePermanentlyRequest) {
                      onDeletePermanentlyRequest();
                    } else if (confirm(`Bạn có chắc muốn xóa vĩnh viễn "${asset.title}"?`)) {

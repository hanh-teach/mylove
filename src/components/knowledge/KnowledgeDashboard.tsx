...
export const KnowledgeDashboard: React.FC<KnowledgeDashboardProps> = ({ 
  ...
  const contextQuality = items.length === 0 
    ? '--' 
    : `${Math.round((items.filter(i => (i.relationships?.length || 0) > 0 || (i.aiNotes?.length || 0) > 0).length / items.length) * 100)}%`;

  const lastSyncText = React.useMemo(() => {
    const timestamps = items.map(i => new Date(i.updatedAt || i.createdAt || 0).getTime());
    ...
    if (validTimestamps.length === 0) return 'Chưa đồng bộ';
    const maxTime = Math.max(...validTimestamps);
    const diffMins = Math.floor((Date.now() - maxTime) / 60000);
    if (diffMins < 1) return 'Vừa xong';
    if (diffMins < 60) return `${diffMins} phút trước`;
    const diffHours = Math.floor(diffMins / 60);
    if (diffHours < 24) return `${diffHours} giờ trước`;
    return `${Math.floor(diffHours / 24)} ngày trước`;
  }, [items, project?.updatedAt]);
          ...
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-[32px] p-8 text-white shadow-2xl relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
                <MemoryStat icon={<Activity size={14} />} label="Context Quality" value={contextQuality} />
                ...
                <MemoryStat icon={<Clock size={14} />} label="Last Sync" value={lastSyncText} />
              </div>
            </div>
          </div>

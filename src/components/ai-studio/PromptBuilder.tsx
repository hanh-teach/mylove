...
export const PromptBuilder: React.FC<PromptBuilderProps> = ({
  ...
  useEffect(() => {
    let isMounted = true;
    idbAdapter.get<any>(DRAFT_STORAGE_KEY)
      .then((parsed) => {
        if (!isMounted) return;
        if (parsed) {
      ...
      .catch((e) => {
        console.warn('Failed to load prompt draft from IndexedDB:', e);
      })
      .finally(() => {
...
- Chi tiết bổ sung từ người dùng: ${customNotes || 'Tập trung thể hiện sự chân thành, ấm áp và ý nghĩa.'}`;
  ...
  const handleSaveDraft = useCallback(() => {
    ...
    idbAdapter.set(DRAFT_STORAGE_KEY, draftData).catch(e => {
  ...
  return (
    <div className="bg-white/95 backdrop-blur-md rounded-3xl border border-rose-100/90 shadow-xs p-5 space-y-4">
      {isDraftLoading && (
        <div className="flex items-center justify-center py-2 text-xs text-rose-500 font-medium bg-rose-50/50 rounded-xl">
          <RefreshCw size={14} className="animate-spin mr-1.5" /> Đang tải bản nháp từ IndexedDB...
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4 text-xs">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <select
              ...
              className="w-full p-2.5 rounded-xl border border-slate-200 bg-slate-50/50 font-medium focus:bg-white focus:ring-2 focus:ring-rose-500 focus:outline-none"
            >
              <option value="Gemini 3.6 Flash">Gemini 3.6 Flash (Siêu tốc & Thông minh)</option>
              <option value="Gemini 3.5 Flash">Gemini 3.5 Flash (Tối ưu & Cân bằng)</option>
              <option value="Gemini 3.5 Flash Lite">Gemini 3.5 Flash Lite (Dự phòng siêu tốc)</option>
              <option value="Gemini 3.1 Pro Preview">Gemini 3.1 Pro Preview (Siêu trí tuệ & Chuyên sâu)</option>
              <option value="Gemini 3.1 Flash Lite">Gemini 3.1 Flash Lite (Phản hồi cực nhanh)</option>
              <option value="NoteMe Creative Agent">NoteMe Creative Agent (Đặc vụ Sáng tạo & Kỷ niệm)</option>
            </select>
          </div>
        </div>
        ...
        {isPendingConfirmation && (
          <div className="p-4 bg-amber-50/90 rounded-2xl border border-amber-200 text-amber-900 text-xs font-bold flex items-start gap-2.5 animate-pulse shadow-xs">
            <Sparkles size={16} className="text-amber-500 shrink-0 mt-0.5" />
            <div className="space-y-1">
              <p className="font-extrabold uppercase text-[10px] tracking-wider text-amber-800">Cần xác nhận ngữ cảnh!</p>
              <p className="font-medium text-slate-700">Bảng phân tích ngữ cảnh <strong>AI Context Inspector</strong> đã được mở ở bên phải màn hình. Vui lòng nhấn nút <strong>"Xác nhận ngữ cảnh"</strong> ở thanh trượt bên phải để bắt đầu quá trình sinh văn bản bằng mô hình AI đã chọn.</p>
            </div>
          </div>
        )}

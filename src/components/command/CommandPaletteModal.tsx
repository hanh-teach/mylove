  ...
  useEffect(() => {
    const defaultCommands: CommandDefinition[] = [
      // 1. Quick Actions
      {
        ...
        description: 'Tạo một dự án câu chuyện kỷ niệm hay thiệp chúc mừng mới',
      ...
      {
        id: 'nav-timeline',
        title: 'Mở Dòng thời gian kỷ niệm (Open Life Timeline 2.0)',
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[10vh] px-4 overflow-hidden">
        ...
        <motion.div
          ...
          className={`relative bg-surface border border-border-subtle text-text-main flex flex-col z-10 shadow-2xl overflow-hidden
            ${deviceType === 'ios' 
        ...
        >
          {/* Header Search Field */}
          <div className="flex items-center gap-3 px-4 py-3.5 border-b border-border-subtle/80 bg-surface-elevated/50">
            ...
            <input
              ...
              className="w-full bg-transparent text-sm border-none focus:outline-none focus:ring-0 text-text-main placeholder-text-muted font-medium"
            />
            {query && (
              <button 
                onClick={() => setQuery('')}
                className="p-1 rounded-full text-text-muted hover:bg-surface-elevated hover:text-text-main"
              >
                <X size={14} />
              </button>
            )}
            <div className="hidden sm:flex items-center gap-1.5 shrink-0">
              <span className="text-[10px] bg-surface-elevated px-2 py-0.5 rounded text-text-muted font-mono flex items-center gap-1 border border-border-subtle">
                <Command size={10} /> Esc
              </span>
            </div>
          </div>
          ...
          <div 
            ref={scrollableRef}
            className="flex-1 overflow-y-auto py-2 divide-y divide-border-subtle/20 max-h-[45vh]"
          >
            {results.sections.length === 0 ? (
              <div className="px-6 py-12 text-center text-text-muted flex flex-col items-center justify-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-surface-elevated flex items-center justify-center text-text-main border border-border-subtle">
                  <Search size={20} />
                </div>
                <div className="text-sm font-semibold text-text-main">Không tìm thấy kết quả nào</div>
              ...
              results.sections.map((sec) => (
                <div key={sec.title} className="py-2 first:pt-1">
                  {/* Category Header */}
                  <div className="px-4 py-1.5 text-[10px] font-extrabold uppercase tracking-widest text-rose-500 flex items-center justify-between">
                    <span>{sec.title}</span>
                    <span className="text-text-muted font-mono font-medium">{sec.items.length}</span>
                  </div>
                  ...
                  <div className="mt-1 space-y-0.5">
                    {sec.items.map((item) => {
                      ...
                      if (sec.type === 'command') {
                        ...
                        tagBadge = (
                          <span className="text-[9px] font-bold bg-rose-500/10 border border-rose-500/20 text-rose-500 px-2 py-0.5 rounded-full">
                            Command
                          </span>
                        );
                      } else if (sec.type === 'project') {
                        titleStr = `${item.icon || '📁'} ${item.title}`;
                        descStr = item.description || 'Dự án NoteMe';
                        iconEl = <Folder className="text-text-muted" size={16} />;
                        tagBadge = (
                          <span className="text-[9px] font-bold bg-indigo-500/10 border border-indigo-500/20 text-indigo-500 px-2 py-0.5 rounded-full">
                            Project
                          </span>
                        );
                        ...
                        iconEl = <BookOpen className="text-rose-500" size={16} />;
                        tagBadge = (
                          <span className="text-[9px] font-bold bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 px-2 py-0.5 rounded-full">
                            Memory
                          </span>
                        );
                        ...
                        titleStr = item.title;
                        descStr = item.description || '';
                        iconEl = <Clock className="text-amber-500" size={16} />;
                        tagBadge = (
                          <span className="text-[9px] font-bold bg-amber-500/10 border border-amber-500/20 text-amber-500 px-2 py-0.5 rounded-full">
                            Timeline
                          </span>
                        );
                      }
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-4 py-2 flex items-center justify-between gap-3 transition-all duration-150 relative border-l-2
                            ${isSelected 
                              ? 'bg-surface-elevated text-text-main border-rose-500' 
                              : 'text-text-muted hover:text-text-main border-transparent hover:bg-surface-elevated/30'
                            }`}
                        >
                          <div className="flex items-center gap-3 truncate min-w-0">
                            <div className={`shrink-0 p-1.5 rounded-xl transition-colors
                              ${isSelected ? 'bg-rose-500/10 text-rose-500' : 'bg-surface-elevated text-text-muted'}`}
                            >
                              {iconEl}
                            </div>
                            <div className="truncate text-left">
                              <div className="text-xs font-bold leading-tight flex items-center gap-2">
                                ...
                                {item.shortcut && (
                                  <span className="hidden sm:inline-block text-[9px] bg-surface-elevated px-1 py-0.1 font-mono rounded text-text-muted border border-border-subtle">
                                    {item.shortcut.join('+')}
                                  </span>
                                )}
                              </div>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            {tagBadge}
                            {isSelected && <ChevronRight size={14} className="text-rose-500" />}
                          </div>
                        </button>
                      );
                    })}
          ...
          {/* Footer Guide */}
          <div className="px-4 py-2 border-t border-border-subtle bg-surface/90 text-[10px] text-text-muted flex items-center justify-between font-medium shrink-0">
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1">
                <kbd className="bg-surface-elevated px-1 py-0.2 rounded font-mono font-bold text-text-main border border-border-subtle">↑↓</kbd> Di chuyển
              </span>
              <span className="flex items-center gap-1">
                <kbd className="bg-surface-elevated px-1 py-0.2 rounded font-mono font-bold text-text-main border border-border-subtle">Enter</kbd> Chọn
              </span>
            </div>
            <div className="flex items-center gap-1 text-rose-500">
              <Sparkles size={11} />
              <span>NoteMe 4.0 Studio</span>
            </div>
          </div>
        </motion.div>
      </div>

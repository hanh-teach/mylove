...
import { Sparkles, Layers, History, Sliders, X, Check, Eye, Grid, Keyboard, Package, Wand2, Activity, ChevronUp, ChevronDown, Heart, FileText, Folder, CalendarRange, Home, ArrowLeft, ChevronRight, ArrowRight, BookOpen, Loader2 } from 'lucide-react';
...
// Dynamic Code-Splitting: Lazy Loaded Modules for High Performance & Bundle Optimization
const AssetBrowser = lazyWithRetry(() => import('./AssetBrowser'), 'AssetBrowser');
const ExportStudioModal = lazyWithRetry(() => import('./export/ExportStudioModal'), 'ExportStudioModal');
const PrintStudioModal = lazyWithRetry(() => import('./print/PrintStudioModal'), 'PrintStudioModal');
const ShortcutsModal = lazyWithRetry(() => import('./ShortcutsModal'), 'ShortcutsModal');
const EditorWorkflowPanel = lazyWithRetry(() => import('./EditorWorkflowPanel'), 'EditorWorkflowPanel');
const EditorMemoryPanel = lazyWithRetry(() => import('./EditorMemoryPanel'), 'EditorMemoryPanel');
const TimelineStudio = lazyWithRetry(() => import('./TimelineStudio'), 'TimelineStudio');

const ModuleLoadingFallback = () => (
  <div className="w-full h-full min-h-[160px] flex flex-col items-center justify-center gap-2 p-6 text-slate-400">
    <Loader2 size={24} className="animate-spin text-pink-500" />
    <span className="text-xs font-semibold text-slate-400">Đang tải phân hệ...</span>
  </div>
);

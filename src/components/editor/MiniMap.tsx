...
export const MiniMap: React.FC<MiniMapProps> = ({
  ...
  canvasWidth = 1000,
  canvasHeight = 650,
  onPanChange = (_x: number, _y: number) => {},
  onResetZoom = () => {},
}) => {
  const minimapRef = useRef<HTMLDivElement>(null);

  const safeCanvasWidth = canvasWidth || 1000;
  const safeCanvasHeight = canvasHeight || 650;

  const miniWidth = 120;
  const miniHeight = Math.round((miniWidth * safeCanvasHeight) / safeCanvasWidth);

  // Calculate visible box relative to minimap scale
  const scaleX = miniWidth / safeCanvasWidth;
  const scaleY = miniHeight / safeCanvasHeight;
  ...
  return (
    <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-xl border border-rose-100 p-2 flex flex-col gap-1.5 select-none z-30">
      <div className="flex items-center justify-between px-1 text-[10px] text-slate-500 font-medium">
        ...
        <button
          onClick={onResetZoom}
          className="hover:text-rose-700 text-rose-600 font-semibold transition-colors flex items-center gap-1"

...
import { EmojiStickerPickerModal } from './toolbar/EmojiStickerPickerModal';
...
import { ComposingInput } from '../../shared/components/ComposingInput';
import { TipTapBlockEditor } from './TipTapBlockEditor';
import { sanitizeHtml, extractPlainText, migrateLegacyContent } from '../../utils/sanitize';
import { AssetPickerModal } from '../asset/AssetPickerModal';
import { SectionErrorBoundary } from '../common/TabErrorBoundary';

interface RichTextEditorProps {
  initialDocument?: NoteDocument;
  onOpenAIAssistant?: () => void;
  onSyncToCard?: (title: string, fullContent: string, images: string[]) => void;
}
...
const RichTextEditorContent: React.FC<RichTextEditorProps> = ({
  ...
  // Sync state when initialDocument prop updates
  useEffect(() => {
    if (initialDocument) {
  ...
  const [isEmojiStickerModalOpen, setIsEmojiStickerModalOpen] = useState(false);
  const [emojiStickerModalTab, setEmojiStickerModalTab] = useState<'emojis' | 'stickers'>('stickers');
  ...
  // Cleanup autosave & version timers on unmount
  useEffect(() => {
    return () => {
  ...
  const handleTitleChange = (newTitle: string) => {
    const updated = { ...doc, title: (newTitle || '').normalize('NFC'), updatedAt: Date.now() };
    updateDocument(updated);
  };

  const handleBlockContentChange = (id: string, content: string) => {
    const norm = (content || '').normalize('NFC');
    const blocks = doc.blocks.map((b) => (b.id === id ? { ...b, content: norm } : b));
  ...
  const getSelectedText = () => {
    ...
    // Extract plain text from HTML safely using DOMParser
    if (content) {
      try {
  ...
  const handleReplaceSelection = (newText: string) => {
    if (selectedBlockId) {
      const blocks = doc.blocks.map(b => b.id === selectedBlockId ? { ...b, content: sanitizeHtml(newText) } : b);
      const updated = { ...doc, blocks, updatedAt: Date.now() };
      updateDocument(updated);
    } else if (doc.blocks.length > 0) {
      const blocks = [...doc.blocks];
      blocks[0].content = sanitizeHtml(newText);
      const updated = { ...doc, blocks, updatedAt: Date.now() };
      updateDocument(updated);
    }
  };
  ...
  const plainBlocksText = doc.blocks.map(b => extractPlainText(b.content));
  const fullText = plainBlocksText.join(' ');
  const syncText = plainBlocksText.join('\n\n');
  ...
  const handleSync = () => {
    if (onSyncToCard) {
      const textBlocks = doc.blocks.filter(b => b.type !== 'image').map(b => extractPlainText(b.content));
      const textOnly = textBlocks.join('\n\n');
      const images = doc.blocks.filter(b => b.type === 'image').map(b => b.content);
      onSyncToCard(doc.title, textOnly, images);
    }
  };
  ...
  return (
    <div className="w-full max-w-[1300px] mx-auto flex flex-col lg:flex-row gap-6 items-start relative my-6 px-4">
      {/* Main Editor Column */}
      <div className="flex-1 w-full bg-white/95 backdrop-blur-md rounded-2xl shadow-xl border border-rose-100 flex flex-col relative overflow-hidden h-auto">
        ...
        {/* Rich Toolbar & Dock Panels Bar (Sprint 57A) - Sticky at top */}
        <div className="flex flex-col bg-rose-50/70 border-b border-rose-100 sticky top-0 z-20 backdrop-blur-md">
          <div className="flex items-center justify-between">
            <div className="flex-1 overflow-x-auto">
              <RichToolbar
                ...
                onOpenEmojiModal={() => {
                  setEmojiStickerModalTab('emojis');
                  setIsEmojiStickerModalOpen(true);
                }}
                onOpenStickerModal={() => {
                  setEmojiStickerModalTab('stickers');
                  setIsEmojiStickerModalOpen(true);
                }}
              />
            </div>
          ...
          {/* Optional Helper Tools Toggle Bar */}
          <div className="bg-slate-50/90 border-t border-rose-100/60 px-4 py-1.5 flex items-center justify-between text-xs text-slate-500 gap-2 overflow-x-auto">
            <div className="flex items-center gap-2">
              <span className="font-semibold text-[11px] text-slate-400 flex items-center gap-1">
                <Sparkles size={13} className="text-rose-500" /> Mở rộng:
              </span>
              <button
                onClick={() => setIsAIPanelOpen(!isAIPanelOpen)}
                className={`px-2.5 py-1 rounded-lg font-medium text-xs flex items-center gap-1 transition-all ${
                  isAIPanelOpen ? 'bg-rose-500 text-white shadow-xs' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-100'
                }`}
              >
                Trợ lý AI {isAIPanelOpen ? '✓' : ''}
              </button>
              <button
                onClick={() => setIsMemoryPanelOpen(!isMemoryPanelOpen)}
                className={`px-2.5 py-1 rounded-lg font-medium text-xs flex items-center gap-1 transition-all ${
                  isMemoryPanelOpen ? 'bg-rose-500 text-white shadow-xs' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-100'
                }`}
              >
                Kỷ niệm {isMemoryPanelOpen ? '✓' : ''}
              </button>
            </div>
            <span className="text-[10px] text-slate-400 hidden sm:inline">Chỉ bật công cụ khi cần thiết</span>
          </div>
        </div>

        {/* 2. Editor Body - Top aligned, expands downwards naturally with content */}
        <div className="p-6 sm:p-8 flex-1 flex flex-col justify-start gap-4 h-auto">
          {/* Document Title */}
          <div className="mb-2">
            <ComposingInput
              type="text"
            ...
            {doc.blocks.map((block, index) => {
              ...
              return (
                <div
                  ...
                  className={`group relative rounded-xl transition-all border p-3 sm:p-4 ${
                ...
                >
                  ...
                  {block.type === 'divider' ? (
                    ...
                    <TipTapBlockEditor
                      content={migrateLegacyContent(block.content)}
                      onChange={(html) => handleBlockContentChange(block.id, sanitizeHtml(html))}
                    ...
                    <TipTapBlockEditor
                      ...
                      className="w-full font-sans italic text-rose-800 bg-rose-50/50 p-3 rounded-lg border-l-4 border-rose-400 focus:outline-none"
                    />
                  ) : block.type === 'checklist' ? (
                    <div className="flex items-center gap-2">
                      <input type="checkbox" className="w-4 h-4 text-rose-600 rounded border-rose-300 focus:ring-rose-500 shrink-0" />
                      <TipTapBlockEditor
                        content={migrateLegacyContent(block.content)}
                        onChange={(html) => handleBlockContentChange(block.id, sanitizeHtml(html))}
                        onBlur={handleBlockBlur}
                        isSingleLine={true}
                        className="w-full font-sans text-slate-800 bg-transparent focus:outline-none text-base"
                      />
                    </div>
                  ) : (
                    <TipTapBlockEditor
                      ...
                      className="w-full font-sans text-slate-800 bg-transparent focus:outline-none leading-relaxed placeholder:text-rose-300 text-base"
                    />
                  )}
                </div>
              );
      ...
      {isAIPanelOpen && (
        <div className="w-full lg:w-[360px] shrink-0 sticky top-4 z-30">
          <SectionErrorBoundary sectionName="Trợ lý AI">
            <AIWritingPanel
      ...
      {isMemoryPanelOpen && (
        <div className="hidden lg:block w-[320px] shrink-0 sticky top-4 bg-white/95 backdrop-blur-md rounded-2xl shadow-xl border border-rose-100 overflow-hidden h-[calc(100vh-80px)]">
          <SectionErrorBoundary sectionName="Thư viện Kỷ niệm">
            <MemoryPanel
      ...
      {/* Version History Panel Modal Overlay */}
      {isVersionHistoryOpen && (
        <div 
          className="fixed inset-0 bg-black/50 backdrop-blur-xs z-50 flex justify-end animate-fadeIn"
      ...
      <AssetPickerModal
        ...
        title="Chọn ảnh từ Thư viện & Supabase Storage"
      />

      {/* Emoji & Sticker Picker Modal at Root Level */}
      <EmojiStickerPickerModal
        isOpen={isEmojiStickerModalOpen}
        onClose={() => setIsEmojiStickerModalOpen(false)}
        onSelectEmoji={(emoji) => {
          handleInsertEmoji(emoji);
          setIsEmojiStickerModalOpen(false);
        }}
        onSelectSticker={(sticker) => {
          handleInsertSticker(sticker);
          setIsEmojiStickerModalOpen(false);
        }}
        initialTab={emojiStickerModalTab}
      />
    </div>
  );
};

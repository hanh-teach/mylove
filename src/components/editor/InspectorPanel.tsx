...
const FONT_OPTIONS = [
  ...
  { id: 'lobster', label: 'Lobster (Nổi bật & Đậm đà)' },
  { id: 'merriweather', label: 'Merriweather (Thanh lịch & Chuyên nghiệp)' },
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
        ...
        {activeTab === 'styles' && (
          <div className="space-y-4">
            ...
            {/* Font Library */}
            <div className="border border-border-subtle rounded-xl bg-surface shadow-2xs p-2.5">
              <div className="flex items-center justify-between font-bold text-text-main mb-2">
                <div className="flex items-center gap-1.5">
                  <Type size={14} className="text-blue-500" />
                  <span>Font Library (Toàn cục)</span>
                </div>
                <span className="text-[10px] text-text-muted font-normal">
                  {FontRegistry.getById(canvasConfig.fontStyle)?.name || canvasConfig.fontStyle}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-1.5 max-h-48 overflow-y-auto pr-0.5">
                {FontRegistry.getAll().map((f) => {
                  const isSelected = canvasConfig.fontStyle === f.id;
                  return (
                    <button
                      key={f.id}
                      type="button"
                      onClick={() => onUpdateCanvasConfig('fontStyle', f.id)}
                      className={`p-2 rounded-lg text-left transition-all flex flex-col justify-between border cursor-pointer ${
                        isSelected
                          ? 'bg-rose-50 dark:bg-rose-950/30 border-rose-500 text-rose-700 dark:text-rose-300 font-semibold shadow-2xs'
                          : 'bg-surface-elevated hover:bg-surface-hover border-border-base text-text-main'
                      }`}
                      title={`${f.name} (${f.category})`}
                    >
                      <div className="flex items-center justify-between w-full">
                        <span className="text-[11px] truncate font-medium">{f.name}</span>
                        {isSelected && <span className="text-[10px] text-rose-500 font-bold">✓</span>}
                      </div>
                      <div className={`text-xs mt-1 truncate ${f.fontClass} opacity-90`}>
                        {f.sample || 'Aa - Love Note'}
                      </div>
                    </button>
                  );
                })}

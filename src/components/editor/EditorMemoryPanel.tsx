...
import { UploadDialog } from '../asset/UploadDialog';
...
export const EditorMemoryPanel: React.FC<EditorMemoryPanelProps> = ({
  ...
  return (
    <div className={`flex flex-col h-full bg-surface text-text-main ${isMobileBottomSheet ? 'p-3' : 'p-4'}`}>
      {/* 1. Header & Actions */}
      <div className="flex items-center justify-between pb-3 mb-3 border-b border-border-subtle">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-text-main leading-tight flex items-center gap-1.5">
              Memory Workspace
              <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-rose-500/10 text-rose-500 dark:text-rose-400 font-semibold">
                {memories.length}
              </span>
            </h3>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="p-1.5 text-text-muted hover:text-text-main hover:bg-surface-elevated rounded-lg transition-colors"
              title="Đóng panel"
            >
              <X size={16} />
            </button>
      ...
      <div className="space-y-2 mb-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 rounded-xl bg-surface-elevated text-text-main border border-border-base text-xs focus:outline-none focus:ring-1 focus:ring-rose-500"
          />
          {searchQuery && (
            <button
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-full font-semibold transition-all shrink-0 ${
              selectedTypeFilter === 'all'
                ...
                : 'bg-surface-elevated text-text-muted hover:bg-surface-hover hover:text-text-main border border-border-subtle'
            }`}
          >
            Tất cả
          </button>
        ...
        <div className="flex items-center justify-between pt-1 text-[11px] text-text-muted border-t border-border-subtle">
          <div className="flex items-center gap-1">
            <button
              onClick={() => setViewMode('grid')}
              className={`p-1 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-rose-500/10 text-rose-500 font-bold' : 'hover:bg-surface-hover'}`}
            ...
            <button
              ...
              className={`p-1 rounded-md transition-colors ${viewMode === 'list' ? 'bg-rose-500/10 text-rose-500 font-bold' : 'hover:bg-surface-hover'}`}
              title="List View"
            >
              <ListIcon size={13} />
            </button>
            <button
              ...
              className={`p-1 rounded-md transition-colors ${viewMode === 'timeline' ? 'bg-rose-500/10 text-rose-500 font-bold' : 'hover:bg-surface-hover'}`}
              title="Timeline View"
            >
              <Clock size={13} />
            </button>
      ...
      <div className="bg-rose-500/5 border border-rose-500/20 rounded-xl px-3 py-2 flex items-center gap-2 mb-3 shrink-0">
      ...
      <div className="flex-1 overflow-y-auto pr-1 space-y-2.5">
        {isLoading ? (
          <div className="space-y-3 py-6">
            {[1, 2, 3].map((n) => (
              <div key={n} className="p-3 rounded-xl border border-border-subtle bg-surface animate-pulse space-y-2">
          ...
          <div className="p-6 text-center space-y-3 bg-red-500/5 rounded-xl border border-red-500/20 text-red-500">
          ...
          <div className="p-6 text-center space-y-3 bg-surface rounded-xl border border-dashed border-border-strong">
            <div className="w-10 h-10 mx-auto rounded-full bg-rose-500/10 flex items-center justify-center text-rose-500">
              <Heart size={20} />
            </div>
            <div>
              <h4 className="font-bold text-text-main text-xs">No memories found</h4>
          ...
          // TIMELINE VIEW
          <div className="relative pl-4 border-l-2 border-border-base space-y-4 py-2">
            {filteredMemories.map((m) => (
              ...
              >
                {/* Dot */}
                <div className="absolute -left-[21px] top-1.5 w-3 h-3 rounded-full bg-rose-500 border-2 border-surface shadow-xs group-hover:scale-125 transition-transform"></div>

                <div className="p-3 rounded-xl bg-surface border border-border-subtle hover:border-rose-400/50 shadow-2xs transition-all space-y-1.5">
                  ...
                  <h4 className="font-bold text-text-main text-xs group-hover:text-rose-600 transition-colors">
                    {m.title}
                  </h4>
                  <p className="text-[11px] text-text-muted line-clamp-2">{m.content}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {filteredMemories.map((m) => (
              <motion.div
                ...
                className="group relative rounded-xl overflow-hidden bg-surface border border-border-subtle hover:border-rose-400/50 shadow-2xs cursor-pointer flex flex-col"
              >
                {m.coverImage ? (
                  <div className="h-28 w-full relative overflow-hidden bg-surface-elevated">
                    <img
                      src={m.coverImage}
                      alt={m.title}
                  ...
                  <div className="h-20 bg-gradient-to-br from-rose-100 to-pink-50 p-2.5 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-full transition-all ${
                        m.isFavorite ? 'bg-rose-500 text-white' : 'bg-surface/80 text-text-main'
                      }`}
                    >
                      <Star size={12} className={m.isFavorite ? 'fill-white' : ''} />
                    </button>
                ...
                <div className="p-2.5 flex-1 flex flex-col justify-between space-y-1">
                  <div>
                    ...
                    <h4 className="font-bold text-text-main text-xs line-clamp-1 group-hover:text-rose-600 transition-colors">
                      {m.title}
                    </h4>
                  </div>
                  <div className="flex items-center justify-between pt-1 border-t border-border-subtle text-[10px]">
                    <span className="text-text-muted truncate max-w-[80px] flex items-center gap-0.5">
                      <MapPin size={9} /> {m.location}
                    </span>
                    <span className="px-1 py-0.5 rounded bg-rose-500/10 text-rose-500 font-semibold text-[9px]">
                      Used by AI 12x
                    </span>
                  </div>
                </div>
          ...
          <div className="space-y-2">
            {filteredMemories.map((m) => (
              <motion.div
                ...
                className="p-3 rounded-xl bg-surface border border-border-subtle hover:border-rose-400/50 shadow-2xs cursor-pointer flex items-center gap-3 transition-all"
              >
                {m.coverImage ? (
                  <img
                    src={m.coverImage}
                  ...
                  <div className="w-14 h-14 rounded-lg bg-rose-500/10 flex items-center justify-center text-xl shrink-0">
                    {getMoodEmoji(m.mood)}
                  </div>
                )}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`p-1 rounded-full transition-all ${
                        m.isFavorite ? 'text-rose-500' : 'text-text-muted hover:text-text-main'
                      }`}
                    >
                      <Star size={14} className={m.isFavorite ? 'fill-rose-500' : ''} />
                    </button>
                  </div>
                  <h4 className="font-bold text-text-main text-xs truncate hover:text-rose-600">
                    {m.title}
                  </h4>
                  <p className="text-[11px] text-text-muted line-clamp-1">{m.content}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        {isDetailOpen && activeMemory && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-xs p-4">
            <motion.div
              ...
              className="bg-surface rounded-2xl shadow-2xl border border-border-base w-full max-w-lg max-h-[90vh] overflow-y-auto flex flex-col"
            >
              ...
              <div className="p-4 space-y-3.5 text-xs">
                {!activeMemory.coverImage && (
                  <div className="flex items-center justify-between pb-2 border-b border-border-subtle">
                    <h2 className="text-sm font-bold text-text-main">{activeMemory.title}</h2>
                    <button
                      onClick={() => setIsDetailOpen(false)}
                ...
                {/* Metadata Row */}
                <div className="grid grid-cols-2 gap-2 bg-surface-elevated p-2.5 rounded-xl border border-border-subtle text-[11px]">
                ...
                {activeMemory.aiSummary && (
                  <div className="p-3 rounded-xl bg-indigo-500/5 border border-indigo-500/20 text-indigo-600 dark:text-indigo-400 space-y-1">
                ...
                <div className="space-y-1">
                  <h5 className="font-bold text-text-muted uppercase tracking-wider text-[10px]">
                    Nội dung / Câu chuyện
                  </h5>
                  <div className="p-3 rounded-xl bg-surface-elevated border border-border-base text-text-main whitespace-pre-wrap leading-relaxed">
                ...
                <div className="flex items-center gap-1 flex-wrap pt-1">
                  {activeMemory.tags.map((t) => (
                    <span
                      key={t}
                      className="px-2 py-0.5 rounded-full bg-rose-500/10 text-rose-500 text-[10px] font-semibold"
                    >
                      #{t}
                    </span>
                  ))}
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-border-subtle">
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl bg-surface-elevated hover:bg-surface-hover text-text-main transition-colors"
                    title="Chỉnh sửa"
                  >
                    <Edit3 size={15} />
                  </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-xs p-4">
            ...
            >
              ...
              <form onSubmit={handleSaveMemory} className="p-4 space-y-3.5 text-xs">
                <div>
                  <label className="block font-semibold text-text-muted mb-1">Tiêu đề kỷ niệm *</label>
                  <input
                    type="text"
                    required
                    value={formTitle}
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-surface-elevated text-text-main border border-border-base focus:outline-none focus:ring-1 focus:ring-rose-500"
                  />
                </div>

                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block font-semibold text-text-muted mb-1">Ngày diễn ra</label>
                    <input
                      type="date"
                      value={formDate}
                  ...
                  <div>
                    <label className="block font-semibold text-text-muted mb-1">Địa điểm</label>
                    <input
                      type="text"
                      value={formLocation}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block font-semibold text-text-muted mb-1">Cảm xúc</label>
                    <select
                      value={formMood}
                  ...
                  <div>
                    <label className="block font-semibold text-text-muted mb-1">Loại kỷ niệm</label>
                    <select
                      value={formType}
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="block font-semibold text-text-muted text-xs">Ảnh bìa & Media</label>
                    <div className="flex items-center gap-1.5">
                      <button
                        type="button"
                        onClick={() => setIsAssetPickerOpen(true)}
                        className="px-2.5 py-1 bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 rounded-lg text-[11px] font-bold flex items-center gap-1 transition-all"
                      >
                        <ImageIcon size={12} />
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 rounded-lg text-[11px] font-bold flex items-center gap-1 transition-all"
                      >
                        <Upload size={12} />
                        <span>Tải lên máy / Supabase</span>
                      </button>
                    </div>
                  </div>
                  <div className="flex gap-2 items-center">
                    <input
                      ...
                      className="flex-1 px-3 py-2 rounded-xl bg-surface-elevated text-text-main border border-border-base focus:outline-none focus:ring-1 focus:ring-rose-500 text-xs"
                    />
                    {formCoverImage && (
                      <div className="w-10 h-10 rounded-xl overflow-hidden border border-border-subtle shrink-0 bg-surface-elevated shadow-xs">
                ...
                <div>
                  <label className="block font-semibold text-text-muted mb-1">Nội dung câu chuyện</label>
                  <textarea
                    rows={4}
                    value={formContent}
                ...
                <div>
                  <label className="block font-semibold text-text-muted mb-1">Thẻ (phân cách bằng dấu phẩy)</label>
                  <input
                    type="text"
                    value={formTags}
                ...
                <div className="flex items-center justify-end gap-2 pt-2 border-t border-border-subtle">
                  <button
                    type="button"
                    onClick={() => setIsCreateOpen(false)}
                    className="px-4 py-2 rounded-xl bg-surface-elevated hover:bg-surface-hover text-text-main font-semibold transition-colors border border-border-base"
                  >
                    Hủy
                  </button>
                  <button
      ...
      <AssetPickerModal
        ...
        onSelectAsset={(asset) => {
          setFormCoverImage(asset.url);
          setIsAssetPickerOpen(false);
        }}
        title="Chọn ảnh bìa từ Thư viện & Supabase"
      />
      ...
      <UploadDialog
        ...
        onUploaded={() => {
          // Auto select latest uploaded asset if needed
        }}
      />
    </div>
  );

...
import { useAssets } from '../../modules/asset/AssetZustandStore';
...
export const TimelineStudio: React.FC<TimelineStudioProps> = ({
  ...
  if (!activeProject || !timeline) {
    return (
      <div className="p-8 text-center bg-surface rounded-3xl border border-border-subtle shadow-xs max-w-md mx-auto mt-10">
        <AlertCircle className="mx-auto text-rose-500 mb-3" size={32} />
        <h3 className="font-bold text-text-main text-sm">Chưa có dự án nào được mở</h3>
        <p className="text-xs text-text-muted mt-1">Hãy chọn hoặc tạo mới một dự án trong Workspace để bắt đầu lập lộ trình Timeline.</p>
      </div>
    );
  }

  return (
    <div className="w-full flex flex-col h-full bg-surface-elevated overflow-hidden relative">
      {/* Top Controller Bar */}
      <div className="p-4 bg-surface border-b border-border-subtle flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shrink-0">
        <div>
          <h2 className="font-black text-text-main text-sm sm:text-base flex items-center gap-2">
            <CalendarRange size={18} className="text-rose-500" />
            <span>Timeline Studio: {timeline.title}</span>
          </h2>
          <p className="text-[11px] text-text-muted mt-0.5">Lộ trình Story-Builder kết nối nội dung, kỷ niệm và trí tuệ nhân tạo AI</p>
        </div>

        {/* Device view switcher & Add Event */}
        <div className="flex items-center gap-2 w-full sm:w-auto self-end">
          <div className="bg-surface-elevated p-0.5 rounded-xl flex items-center gap-0.5 text-text-muted border border-border-subtle">
            <button 
              onClick={() => setLayoutMode('desktop')}
              className={`p-1.5 rounded-lg text-xs font-bold flex items-center gap-1 transition-all ${layoutMode === 'desktop' ? 'bg-surface text-text-main shadow-xs' : 'hover:text-text-main'}`}
            ...
            <button 
              ...
              className={`p-1.5 rounded-lg text-xs font-bold flex items-center gap-1 transition-all ${layoutMode === 'android' ? 'bg-surface text-text-main shadow-xs' : 'hover:text-text-main'}`}
            ...
            <button 
              ...
              className={`p-1.5 rounded-lg text-xs font-bold flex items-center gap-1 transition-all ${layoutMode === 'ios' ? 'bg-surface text-text-main shadow-xs' : 'hover:text-text-main'}`}
              title="iOS Cards"
            >
      ...
      <div className="px-4 py-3 bg-surface border-b border-border-subtle flex items-center justify-between gap-4 shrink-0 shadow-2xs">
        <div className="flex-1 flex items-center gap-3">
          <span className="text-xs font-extrabold text-text-main min-w-[50px]">Tiến trình:</span>
          <div className="flex-1 h-2.5 bg-surface-elevated rounded-full overflow-hidden relative border border-border-subtle">
            <motion.div 
        ...
        {/* AI Active Indicator */}
        {activeStepForAI && (
          <div className="hidden lg:flex items-center gap-2 bg-surface-elevated text-text-main rounded-xl py-1 px-2.5 border border-border-base text-[11px] shrink-0">
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        ...
        <div className="p-3 bg-surface border-b border-border-subtle flex flex-col md:flex-row items-center justify-between gap-3 shrink-0">
          <div className="relative w-full md:w-64">
            ...
            <input
              ...
              className="w-full pl-8 pr-4 py-1.5 bg-surface-elevated border border-border-base rounded-lg text-xs focus:outline-hidden focus:ring-1 focus:ring-rose-400 text-text-main"
            />
          </div>

          <div className="flex items-center gap-1 w-full md:w-auto overflow-x-auto py-1">
            <FilterIcon size={12} className="text-text-muted mr-1 shrink-0" />
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-[10px] font-bold capitalize transition-all shrink-0 ${
                  activeFilter === filter 
                    ...
                    : 'bg-surface-elevated hover:bg-surface-hover text-text-muted hover:text-text-main border border-border-subtle'
                }`}
              >
        ...
        <div className="flex-1 overflow-y-auto p-4 flex justify-center">
          ...
          {layoutMode === 'desktop' && (
            <div className="w-full max-w-4xl space-y-6">
              {filteredItems.length === 0 ? (
                <div className="p-12 text-center bg-surface rounded-3xl border border-border-subtle shadow-2xs">
                  <CalendarRange className="mx-auto text-text-muted opacity-60 mb-2" size={36} />
                  <p className="text-xs text-text-muted font-semibold">Không tìm thấy bước nào trong Timeline phù hợp.</p>
                </div>
              ) : (
                <div className="relative pl-6 sm:pl-8 border-l-2 border-dashed border-border-base space-y-6">
                  {filteredItems.map((item, index) => {
                    ...
                    return (
                      <div 
                        ...
                        className={`relative group bg-surface rounded-2xl border transition-all p-4 ${
                          item.completed 
                            ? 'border-emerald-500/20 bg-emerald-500/5 shadow-2xs' 
                            : 'border-border-subtle shadow-2xs hover:shadow-xs hover:border-rose-400/50'
                        } ${draggedItemId === item.id ? 'opacity-40 border-dashed border-rose-400' : ''}`}
                      >
                        {/* Bullet bullet connection */}
                        <div 
                          onClick={() => handleToggleComplete(item.id, item.completed)}
                          className={`absolute -left-10 sm:-left-12 top-4 w-8 h-8 rounded-full border-4 border-surface flex items-center justify-center cursor-pointer transition-all shadow-md z-10 ${
                            item.completed 
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="space-y-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              <h3 className={`font-bold text-text-main text-xs sm:text-sm ${item.completed ? 'line-through text-text-muted' : ''}`}>
                                {item.title}
                              </h3>
                              {item.aiGenerated ? (
                                <span className="px-1.5 py-0.5 rounded-md text-[9px] font-extrabold bg-indigo-500/10 text-indigo-500 dark:text-indigo-400 border border-indigo-500/20 flex items-center gap-0.5">
                                  <Sparkles size={8} className="fill-indigo-300" />
                                  <span>AI Step</span>
                                </span>
                              ) : (
                                <span className="px-1.5 py-0.5 rounded-md text-[9px] font-extrabold bg-surface-elevated text-text-muted border border-border-subtle">
                                  Manual
                                </span>
                              )}
                            </div>
                            <p className="text-[11px] text-text-muted font-medium whitespace-pre-wrap">{item.description}</p>
                          </div>
                          ...
                          <div className="flex items-center gap-1 shrink-0 opacity-80 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              disabled={index === 0}
                              className="p-1 rounded-md hover:bg-surface-hover text-text-muted disabled:opacity-20 transition-all"
                              title="Di chuyển lên"
                            >
                              <ArrowUp size={12} />
                            </button>
                            ...
                            <button
                              ...
                              className="p-1 rounded-md hover:bg-rose-500/10 text-rose-500 transition-all"
                              title="Sửa"
                            >
                              <FileText size={12} />
                            </button>
                            <button
                              ...
                              className="p-1 rounded-md hover:bg-rose-500/20 text-rose-600 transition-all"
                              title="Xóa"
                            >
                              <Trash2 size={12} />
                            </button>
                        ...
                        {/* Attachments Section */}
                        <div className="mt-3.5 pt-3.5 border-t border-dashed border-border-subtle space-y-2.5">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="flex items-center gap-1.5">
                              <button
                                ...
                                className="px-2 py-0.5 bg-surface-elevated hover:bg-surface-hover text-text-main rounded-md text-[9px] font-bold flex items-center gap-0.5 border border-border-subtle"
                              >
                                <ImageIcon size={9} />
                                <span>+ Asset</span>
                              </button>
                              ...
                              <button
                                ...
                                className="px-2 py-0.5 bg-surface-elevated hover:bg-rose-500/10 hover:text-rose-500 text-text-main rounded-md text-[9px] font-bold flex items-center gap-0.5 border border-border-subtle"
                          ...
                          {((item.assetIds?.length || 0) + (item.memoryIds?.length || 0) + (item.draftId ? 1 : 0)) > 0 && (
                            <div className="flex items-center gap-2 flex-wrap pt-1">
                              ...
                              {item.assetIds?.map(assetId => {
                                ...
                                return (
                                  <div key={assetId} className="flex items-center gap-1.5 bg-surface-elevated border border-border-base rounded-xl pl-1 pr-2 py-1 text-[10px] text-text-main font-semibold group/att max-w-[150px] truncate relative">
                                    {asset.type === 'image' && asset.url ? (
                                      <img src={asset.url} alt="" referrerPolicy="no-referrer" className="w-5 h-5 rounded-md object-cover border border-border-subtle" />
                                    ) : (
                                    ...
                                    <button 
                                      ...
                                      className="p-0.5 hover:bg-surface-hover rounded-full text-text-muted hover:text-rose-600 ml-1 transition-colors"
                                    >
                                      <X size={10} />
                                    </button>
                                  </div>
                              ...
                              {item.memoryIds?.map(memId => {
                                ...
                                if (!mem) return null;
                                return (
                                  <div key={memId} className="flex items-center gap-1 bg-rose-500/10 border border-rose-500/20 rounded-xl pl-1.5 pr-2 py-1 text-[10px] text-rose-600 dark:text-rose-400 font-semibold max-w-[150px] truncate relative">
                                    <Heart size={10} className="text-rose-500 fill-rose-500" />
                                    <span className="truncate">{mem.title}</span>
                                    <button 
                                      onClick={() => handleRemoveAttachment(item.id, 'memory', memId)}
                                      className="p-0.5 hover:bg-rose-500/20 rounded-full text-text-muted hover:text-rose-700 ml-1 transition-colors"
                                    >
                                      <X size={10} />
                                    </button>
                                  </div>
                              ...
                              {/* Attached Draft */}
                              {item.draftId && (
                                <div className="flex items-center gap-1 bg-amber-500/10 border border-amber-500/20 rounded-xl pl-1.5 pr-2 py-1 text-[10px] text-amber-600 dark:text-amber-400 font-semibold max-w-[180px] truncate relative">
                                  <FileText size={10} className="text-amber-600" />
                                  <span 
                                    className="underline cursor-pointer hover:text-amber-500 truncate"
                                  ...
                                  <button 
                                    ...
                                    className="p-0.5 hover:bg-amber-500/20 rounded-full text-text-muted hover:text-amber-700 ml-1 transition-colors"
                                  >
                                    <X size={10} />
                                  </button>
                                </div>
                        ...
                        {aiRec && (
                          <div className="mt-3 bg-surface-elevated p-2.5 rounded-xl border border-border-subtle flex items-center justify-between gap-3 text-[11px] text-text-muted font-medium">
                            <div className="flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              className="px-2.5 py-1 bg-surface hover:bg-rose-500/10 border border-border-base hover:border-rose-300 text-rose-600 font-extrabold rounded-lg shrink-0 transition-all text-[10px]"
                            >
                              {aiRec.actionLabel}
                            </button>
                          </div>
          ...
          {/* ====================================
              ...
              Giữ màu cố định: đây là khung mô phỏng điện thoại xem trước của thiết bị Android, không theo theme app
             ==================================== */}
          {layoutMode === 'android' && (
          ...
          {/* ====================================
              ...
              Giữ màu cố định: đây là khung mô phỏng điện thoại xem trước của thiết bị iOS, không theo theme app
             ==================================== */}
          {layoutMode === 'ios' && (
            <div className="w-full max-w-sm bg-slate-50 text-slate-900 rounded-[40px] border-[10px] border-slate-900 p-6 flex flex-col h-[580px] shadow-2xl relative overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto py-4 space-y-3.5 pr-0.5">
                <div className="flex items-center justify-between mb-1.5 bg-rose-500/10 border border-rose-500/20 p-2.5 rounded-2xl">
                ...
                {items.map((item, index) => (
                  <div 
                    key={item.id} 
                    className="bg-surface rounded-2xl p-3.5 shadow-xs border border-border-subtle flex flex-col gap-2.5"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex gap-2 items-center">
                        <button 
                          ...
                          className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 transition-all ${
                            item.completed 
                              ...
                              : 'border-border-base hover:border-rose-400'
                          }`}
                        >
                        ...
                        <div className="min-w-0">
                          <h4 className={`text-[11px] font-black ${item.completed ? 'line-through text-text-muted' : 'text-text-main'}`}>
                            {item.title}
                          </h4>
                          <span className="text-[9px] text-text-muted font-bold block">Bước {index + 1}</span>
                        </div>
                      </div>

                      {item.aiGenerated && (
                        <span className="px-1.5 py-0.5 rounded-full text-[8px] font-extrabold bg-indigo-500/10 text-indigo-500 dark:text-indigo-400">AI</span>
                      )}
                    </div>

                    <p className="text-[10px] text-text-muted line-clamp-2 leading-relaxed">{item.description}</p>

                    {/* Quick indicator of attached elements */}
                    {((item.assetIds?.length || 0) + (item.memoryIds?.length || 0) + (item.draftId ? 1 : 0)) > 0 && (
                      <div className="flex items-center gap-1.5 pt-2 border-t border-border-subtle text-[9px] text-text-muted font-bold">
                        <Paperclip size={8} />
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs">
          <div className="bg-surface rounded-3xl w-full max-w-md shadow-2xl border border-border-base overflow-hidden">
            ...
            <form onSubmit={handleCreateItem} className="p-4 space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-text-muted block uppercase">Tiêu đề bước</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3 py-2 bg-surface-elevated text-text-main border border-border-base rounded-xl text-xs focus:outline-hidden focus:ring-1 focus:ring-rose-400"
                />
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-text-muted block uppercase">Mô tả / Mục tiêu</label>
                <textarea
                  rows={3}
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  ...
                  className="w-4 h-4 text-rose-500 border-border-base rounded-sm focus:ring-rose-400"
                />
                <label htmlFor="isAiStep" className="text-xs text-text-main font-bold flex items-center gap-1 cursor-pointer">
              ...
              <div className="pt-2 flex justify-end gap-2">
                <button
                  ...
                  className="px-4 py-2 border border-border-base rounded-xl hover:bg-surface-hover text-text-main font-semibold text-xs transition-colors"
                >
                  Hủy
                </button>
                <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs">
          <div className="bg-surface rounded-3xl w-full max-w-md shadow-2xl border border-border-base overflow-hidden">
            <div className="p-4 border-b border-border-subtle flex items-center justify-between bg-surface">
              ...
              <button onClick={() => setIsEditModalOpen(false)} className="text-text-muted hover:text-text-main">
                <X size={16} />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveEditItem} className="p-4 space-y-4">
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  ...
                  className="w-4 h-4 text-rose-500 border-border-base rounded-sm focus:ring-rose-400"
                />
                <label htmlFor="isAiStepEdit" className="text-xs text-text-main font-bold flex items-center gap-1 cursor-pointer">
                  <Sparkles size={12} className="text-indigo-500" />
                  <span>Bước hỗ trợ bởi Trí tuệ Nhân tạo (AI Step)</span>
                </label>
              </div>

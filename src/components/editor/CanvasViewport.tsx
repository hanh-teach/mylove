...
import { SmartGuideResult } from '../../modules/editor/SmartGuideService';
...
import { useCanvasBusiness } from '../../modules/editor/hooks/useCanvasBusiness';
import { 
  ...
  AlignCenter, AlignRight, CheckCircle2, RefreshCw, AlertTriangle
} from 'lucide-react';
...
export const CanvasViewport: React.FC<CanvasViewportProps> = ({
  ...
  // Spacebar pan keyboard listener
  useEffect(() => {
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    if (isSpacePressed || e.button === 1) {
      ...
      if (artboardRef.current) {
        startMarquee(e, artboardRef.current.getBoundingClientRect());
      }
    }
  };

  const handlePointerMove = (e: React.PointerEvent) => {
    const artboardRect = artboardRef.current ? artboardRef.current.getBoundingClientRect() : null;
    updatePointerMove(e, artboardRect);
  };
  ...
  const handleDrop = (e: React.DragEvent) => {
    if (artboardRef.current) {
      handleAssetDrop(e, artboardRef.current.getBoundingClientRect());
    }
  };
  ...
  return (
    <div
      ref={containerRef}
      onWheel={(e) => handleWheel(e, containerRef.current?.getBoundingClientRect() || null)}
      onPointerDown={handlePointerDown}
      onPointerMove={handlePointerMove}
      onPointerUp={endPointer}
      onContextMenu={handleContextMenu}
      // UI Chrome: Background mờ của vùng làm việc (viewport) bao quanh canvas, tự động thích ứng theo theme
      className={`relative w-full h-full overflow-hidden bg-surface-elevated/40 backdrop-blur-3xl select-none ${
    ...
    >
      ...
      >
        ...
        {layers.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 bg-surface/30 z-10 text-center">
            <div className="w-16 h-16 rounded-3xl bg-rose-100 dark:bg-rose-900/30 text-rose-600 flex items-center justify-center mb-4 shadow-sm animate-bounce">
              <Sparkles size={32} />
            </div>
            <h3 className="text-xl font-bold text-text-main mb-1">Writer Studio Canvas Trống</h3>
            ...
            <div className="grid grid-cols-2 gap-3 max-w-md w-full">
              <button
                ...
                className="p-3 bg-surface hover:bg-surface-hover border border-border-base rounded-2xl flex items-center gap-3 text-text-main font-semibold text-xs shadow-sm hover:shadow-md transition-all group"
              >
                <div className="p-2 bg-blue-100 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 rounded-xl group-hover:scale-110 transition-transform">
                  <Type size={18} />
                </div>
              ...
              <button
                ...
                className="p-3 bg-surface hover:bg-surface-hover border border-border-base rounded-2xl flex items-center gap-3 text-text-main font-semibold text-xs shadow-sm hover:shadow-md transition-all group"
              >
                <div className="p-2 bg-rose-100 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 rounded-xl group-hover:scale-110 transition-transform">
                  <Heart size={18} />
                </div>
        ...
        {layers
          ...
          .map((layer) => {
            ...
            // Giữ màu cố định: đây là nội dung thiệp do người dùng thiết kế, không theo theme app
            if (layer.type === 'text') {
            ...
            if (layer.type === 'image') {
              ...
              return (
                ...
                >
                  ...
                  >
                    <img
                      src={layer.metadata?.imageUrl || layer.metadata?.url || (layer as any).url || 'https://images.unsplash.com/photo-1518173946687-a4c8a383392e?auto=format&fit=crop&w=600&q=80'}
                      alt={layer.name}
                      crossOrigin="anonymous"
            ...
            if (layer.type === 'ai_block') {
              ...
              const isSimulated = layer.metadata?.isSimulated;
              ...
              return (
                ...
                >
                  <div className="space-y-2 min-h-0 flex-1 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto bg-white/5 border border-white/5 rounded-lg p-2.5 min-h-[50px] flex flex-col gap-1.5">
                      {isSimulated && (
                        <div className="bg-amber-500/20 border border-amber-500/30 rounded p-1.5 flex items-start gap-1.5 text-amber-200 text-[9px]">
                          <AlertTriangle size={10} className="mt-0.5 shrink-0" />
                          <span className="leading-tight">Bản xem trước - chưa dùng AI thật</span>
                        </div>
                      )}
                      <p className="text-[10px] italic text-slate-200 leading-relaxed leading-normal whitespace-pre-wrap w-full flex-1 flex items-center">
                        {suggestion}
                      </p>
                    </div>
                  </div>

...
import { BrainCircuit, Activity, Database, Sparkles, ChevronRight, ChevronLeft, Play, Clock, Code, Download, Wand2 } from 'lucide-react';
...
import { AIMultimodalStudio } from './AIMultimodalStudio';

interface AISidebarProps {
  onOpenExport?: (tab: 'export' | 'history') => void;
}

export const AISidebar: React.FC<AISidebarProps> = ({ onOpenExport }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<'multimodal' | 'workflow' | 'memory' | 'reflection' | 'inspector'>('multimodal');
  ...
  useEffect(() => {
    const checkSidebar = () => {
      const exists = !!document.getElementById('smart-workflow-sidebar');
      setIsSidebarOpen(exists);
    };

    checkSidebar();
    const interval = setInterval(checkSidebar, 800);
  ...
  return (
    <>
      <div className={`fixed top-32 z-40 flex flex-col gap-3 items-center transition-all duration-300 ${
        isSidebarOpen ? 'right-[360px] lg:right-[380px] max-lg:hidden' : 'right-4'
      }`}>
        {/* AI Button */}
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="w-11 h-11 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-full shadow-lg hover:shadow-indigo-500/25 hover:scale-105 border border-indigo-300/40 transition-all flex items-center justify-center group"
          title="Mở Trợ lý AI"
        >
          <BrainCircuit size={20} className="animate-pulse group-hover:rotate-12 transition-transform" />
        </button>

        {/* Clock Button (Publish History) */}
        {onOpenExport && (
          <button
            onClick={() => onOpenExport('history')}
            className="w-11 h-11 bg-white hover:bg-slate-50 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 rounded-full shadow-lg hover:scale-105 border border-slate-200 dark:border-slate-700 transition-all flex items-center justify-center cursor-pointer"
        ...
        {/* Download Button (Export & Download) */}
        {onOpenExport && (
          <button
            onClick={() => onOpenExport('export')}
            className="w-11 h-11 bg-white hover:bg-slate-50 text-blue-600 dark:text-blue-400 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-full shadow-lg hover:scale-105 border border-slate-200 dark:border-slate-700 transition-all flex items-center justify-center cursor-pointer"
            title="Xuất & Tải về"
          >
            <Download size={20} className="text-blue-700 dark:text-blue-400 font-bold" />
          </button>
        )}
      </div>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {/* Navigation Tabs */}
            <div className="flex p-2 gap-1 bg-gray-50/50 border-b border-gray-100 overflow-x-auto">
              <button
                ...
                className={`flex-1 flex flex-col items-center justify-center py-2 px-1 rounded-lg text-xs font-medium transition-colors ${
                  activeTab === 'multimodal' ? 'bg-white shadow-sm text-pink-600 font-bold' : 'text-gray-500 hover:text-gray-700 hover:bg-white/50'
                }`}
              >
                <Wand2 size={16} className="mb-1 text-pink-500" />
                AI Studio
              </button>
              <button

              ...
              {filteredMarkers.map((marker) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`p-4 rounded-3xl cursor-pointer border-2 transition-all shadow-sm flex items-center gap-3 ${
                      isSelected
                        ? 'bg-rose-500 text-white border-rose-600 shadow-xl scale-105 z-20'
                        : 'bg-white/90 hover:bg-white text-slate-800 border-slate-200/90 hover:shadow-md'
                    }`}
                  >
                    <span
                    ...
                    >
                      <MapPin size={18} fill={isSelected ? 'currentColor' : 'none'} />
                    </span>

                    <div className="truncate">
                      <div className="text-xs font-black truncate tracking-tight">{marker.cityName}</div>
                      <div
                        className={`text-[10px] font-bold ${
                          isSelected ? 'text-white/80' : 'text-slate-500'
                        }`}
                      >
                        {marker.memories.length} kỉ niệm
                      </div>
                    </div>
                  </motion.div>
                );

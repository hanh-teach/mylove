...
export function requestLoggerMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  // Identify internal dev static source requests to avoid cluttering logs
  const isStaticSource = req.path.startsWith('/src/') || 
                         req.path.startsWith('/node_modules/') || 
                         req.path.startsWith('/@vite/') || 
                         req.path.startsWith('/@id/') ||
                         /\.(tsx?|jsx?|css|json|map|png|jpg|jpeg|svg|ico)$/i.test(req.path);

  const module = req.path.startsWith('/api') ? 'backend-api' : 'frontend-serve';

  // Log incoming requests for API routes and non-static assets
  if (!isStaticSource) {
  ...
  // Use native Express finish event instead of overriding res.end
  res.on('finish', () => {
    const duration = Date.now() - (req.startTime || Date.now());
    
    // Only log response logs for API routes, non-static assets, or error statuses (>= 400)
    if (!isStaticSource || res.statusCode >= 400) {
